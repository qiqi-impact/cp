...
struct segment {
    ...
    void apply(const segment_change &change) {
        if (change.has_set()) {
            ...
            mxl = mxr = mxs = max(change.to_set, 0);
            sum = change.to_set;
            max_diff = 0;
        }
    ...
    void join(const segment &other) {
        ...
        mxs = max(max(mxs, other.mxs), mxr + other.mxl);
        mxl = max(mxl, sum + other.mxl);
        mxr = max(other.mxr, other.sum + mxr);
        sum += other.sum;
